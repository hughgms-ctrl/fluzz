-- Update the notify_task_assigned trigger to check if project is in draft mode
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' AND NEW.assigned_to IS NOT NULL) OR 
    ...
    -- Check if project is in draft mode (pending_notifications = true)
    IF NEW.project_id IS NOT NULL THEN
      SELECT pending_notifications INTO _is_draft
      FROM projects
      WHERE id = NEW.project_id;
      
      -- If project is in draft mode, don't send notification
