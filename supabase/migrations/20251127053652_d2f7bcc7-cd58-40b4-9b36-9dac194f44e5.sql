...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    ...
    -- Get project owner to notify them
    IF NEW.project_id IS NOT NULL THEN
      SELECT user_id, workspace_id INTO _project_owner, _workspace_id
      FROM projects
      WHERE id = NEW.project_id;
      
      -- Only notify if the completer is not the project owner
      IF _project_owner != NEW.assigned_to THEN
        INSERT INTO notifications (
          user_id,
          workspace_id,
          type,
          ...
          'task_completed',
          'Tarefa concluída',
          'A tarefa "' || _task_title || '" foi concluída',
          '/tasks/' || NEW.id,
          jsonb_build_object(
            'task_id', NEW.id,
            'task_title', _task_title,
